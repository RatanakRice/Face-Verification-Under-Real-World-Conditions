...
def load_arcface_model():
    ...
    get_model() can't resolve a bare pack name (the pack holds several .onnx
    files), so we download the pack if needed and point it at the recognition net.
    """
...
def embed_arcface(model, image:np.ndarray) -> np.ndarray:
    """Run one aligned image through Arcface, return its embedding vector"""
    resized = cv2.resize(image, (112, 112)) #buffalo_l model expects 112x112 input
    embedding = model.get_feat(resized)
    return embedding.flatten()

def embed_face_recognition(image: np.ndarray):
    """
    Run one aligned image through face_recognition (dlib).
    Since the image is already cropped, we tell it the face location is the whole image
    Returns None if dlib can't find landmarks on the crop.
    """
    ...
    encodings = face_recognition.face_encodings(
        rgb_image, known_face_locations=whole_image_box
    )
    if len(encodings) == 0:
        return None
    return encodings[0]
...
def process_folder(arcface_model, input_dir: Path, output_dir: Path, dataset_name: str):
    """
    Walk input_dir, embed every image with both models, save results as two JSON files.
    """
...
# python src/embed.py --input data/processed/aligned/lfw --output data/processed/embeddings --name lfw
